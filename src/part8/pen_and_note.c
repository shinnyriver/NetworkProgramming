...
// 세마포어 조작
struct sembuf increase[1] = { { 0, +1, SEM_UNDO }, { 1, +1, SEM_UNDO } };
struct sembuf decrease[1] = { { 0, -1, SEM_UNDO }, { 1, -1, SEM_UNDO } };

// 세마포어 초기 값: 연필 한 자루와 노트 두 권
unsigned short seminitval[1] = { 1, 2 };
...
void do_work() {
    int count = 0;
    #define Semop(val) if ((semop(semid, val, 1)) == -1) errquit("semop")
